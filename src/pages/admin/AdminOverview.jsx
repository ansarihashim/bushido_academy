...
function StatCard({ label, value, hint, to, icon, loading }) {
  return (
    <Link
      to={to}
      className="group relative bg-[#171717] border border-neutral-800 hover:border-red-600 rounded-sm p-6 sm:p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_-20px_rgba(220,38,38,0.4)] block"
    >
      <span className="absolute top-0 left-0 h-1 w-0 bg-red-600 group-hover:w-full transition-all duration-500" />
      <div className="flex items-start justify-between mb-6">
        <span className="inline-flex items-center justify-center w-12 h-12 rounded-sm bg-red-950/40 border border-red-900/50 text-red-600 group-hover:bg-red-600 group-hover:text-white transition-all duration-300">
          {icon}
        </span>
...
export default function AdminOverview() {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="mb-6 bg-[#171717] border-l-4 border-red-600 p-4 rounded-sm">
